...
const getBrinquedoById = async (req, res) => {
  const brinquedo = await brinquedosService.getBrinquedoById(Number(req.params.id));
...
const updateBrinquedo = async (req, res) => {
  const brinquedo = await brinquedosService.updateBrinquedo(Number(req.params.id), req.body);
  if (brinquedo) {
    return res.status(200).json(brinquedo);
  } else {
...
const deleteBrinquedo = async (req, res) => {
  const brinquedo = await brinquedosService.deleteBrinquedo(Number(req.params.id));
  if (brinquedo) {
